...
func jumpcloudTestProvider() *schema.Provider {
	provider := &schema.Provider{
		ResourcesMap: map[string]*schema.Resource{
			"jumpcloud_admin_user":         ResourceUser(),
			"jumpcloud_admin_role":         ResourceRole(),
			"jumpcloud_admin_role_binding": ResourceRoleBinding(),
		},
		DataSourcesMap: map[string]*schema.Resource{
			"jumpcloud_admin_users": DataSourceUsers(),
		},
	}
	return provider
}
